...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    // Initially hide the cursor if a container is provided
    if (containerRef) {
    ...
    // Attach move handler to window to ensure smooth tracking even if mouse slightly leaves container
    // But we might want to restrict it if strictly requested.
    // Usually tracking on window is better for UX, but visibility is toggled.
    ...
    const containerMouseEnter = () => {
      if (hideDefaultCursor && containerRef?.current) {
    ...
    if (containerEl) {
      containerEl.addEventListener("mouseenter", containerMouseEnter);
      containerEl.addEventListener("mouseleave", containerMouseLeave);
      // Also attach mouseover to container if provided to scope target detection
      containerEl.addEventListener("mouseover", enterHandler as EventListener);
    } else {
      // Global behavior if no container
    ...
    return () => {
      ...
      if (containerEl) {
        containerEl.removeEventListener("mouseenter", containerMouseEnter);
        containerEl.removeEventListener("mouseleave", containerMouseLeave);
